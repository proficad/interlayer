...
namespace View.Voronoi
{
    partial class Delaynay
    {
        ...
        public void CreateVoronoi(ref List<AreaPoint> AreaPointList)
        //public List<PointF> CreateVoronoi(int Index)
        {
            List<PointF> PointList = new List<PointF>();
            
            //三角型轮廓
            PointF[] Points = new PointF[ch.HullPoints.Count];
            Points = ch.HullPoints.ToArray();
            GraphicsPath PointPath = new GraphicsPath();
            PointPath.AddClosedCurve(Points);

            for (int i = 0; i < DS.TinEdgeNum; i++)
            {
                if (!DS.TinEdges[i].NotHullEdge) //△边为凸壳边
                {
                    DrawHullVorEdge(i,ref AreaPointList);
                    //DrawHullVorEdge(i, ref PointList,Index);
                    continue;
                }
                //连接左/右△的外接圆心
                ...
                //圆心在box外则直接跳过
                for (int j = 0; j < ch.HullPoints.Count; j++)
                {
                    if (!float.IsNaN(p1.X) && !float.IsNaN(p1.Y) && !float.IsNaN(p2.X) && !float.IsNaN(p2.Y) && !float.IsNaN(ch.HullPoints[j].X) && !float.IsNaN(ch.HullPoints[j].Y) && !float.IsNaN(ch.HullPoints[(j + 1) % ch.HullPoints.Count].X) && !float.IsNaN(ch.HullPoints[(j + 1) % ch.HullPoints.Count].Y))
                    {
                        if (CrossPoint.IsLineSegmentCross(p1, p2, ch.HullPoints[j], ch.HullPoints[(j + 1) % ch.HullPoints.Count]))
                        {
                            PointF p = CrossPoint.GetCrossPoint(p1, p2, ch.HullPoints[j], ch.HullPoints[(j + 1) % ch.HullPoints.Count]);
                            if (PointPath.IsVisible(p1))
                            {
                                p2 = p;
                            }
                            else
                ...
                if (PointPath.IsVisible(p1) || PointPath.IsVisible(p2))
                {
                    for (int j = 0; j < AreaPointList.Count; j++)
                    {
                        if (!(DS.TinEdges[i].Vertex1ID != AreaPointList[j].Index && DS.TinEdges[i].Vertex2ID != AreaPointList[j].Index))
                        {
                            if (!AreaPointList[j].PointList.Contains(p1))
                            {
                                AreaPointList[j].PointList.Add(p1);
                            }
                            if (!AreaPointList[j].PointList.Contains(p2))
                            {
                                AreaPointList[j].PointList.Add(p2);
                            }
                        }
                    }
                }
            ...
            //return PointList;
             ...
             //构建Voronoi图
        public List<PointF> CreateVoronoi()
        {
            ...
            for (int i = 0; i < DS.TinEdgeNum; i++)
            {
                ...
                //if (DS.TinEdges[i].Vertex1ID != PointIndex && DS.TinEdges[i].Vertex2ID != PointIndex)
                //{
        ...
        private void DrawHullVorEdge(int i, ref List<AreaPoint> AreaPointList)
        //private void DrawHullVorEdge(int i, ref List<PointF> PointList,int Index)
        {
            ...
            for (int j = 0; j < ch.HullPoints.Count; j++)
            {
                if (!float.IsNaN(BaryCnt.X) && !float.IsNaN(BaryCnt.Y) && !float.IsNaN(EndPnt.X) && !float.IsNaN(EndPnt.Y) && !float.IsNaN(ch.HullPoints[j].X) && !float.IsNaN(ch.HullPoints[j].Y) && !float.IsNaN(ch.HullPoints[(j + 1) % ch.HullPoints.Count].X) && !float.IsNaN(ch.HullPoints[(j + 1) % ch.HullPoints.Count].Y))
                {
                    if (CrossPoint.IsLineSegmentCross(BaryCnt, EndPnt, ch.HullPoints[j], ch.HullPoints[(j + 1) % ch.HullPoints.Count]))
                    {
                        PointF p = CrossPoint.GetCrossPoint(BaryCnt, EndPnt, ch.HullPoints[j], ch.HullPoints[(j + 1) % ch.HullPoints.Count]);
                        if (PointPath.IsVisible(BaryCnt))
                        {
                            EndPnt = p;
                        }
                        else
            ...
            if (PointPath.IsVisible(BaryCnt) || PointPath.IsVisible(EndPnt))
            {
                for (int j = 0; j < AreaPointList.Count; j++)
                {
                    //if (!(DS.TinEdges[i].Vertex1ID != Index && DS.TinEdges[i].Vertex2ID != Index))
                    if (!(DS.TinEdges[i].Vertex1ID != AreaPointList[j].Index && DS.TinEdges[i].Vertex2ID != AreaPointList[j].Index))
                    {
                        if (!AreaPointList[j].PointList.Contains(BaryCnt))
                        {
                            AreaPointList[j].PointList.Add(BaryCnt);
                        }
                        if (!AreaPointList[j].PointList.Contains(EndPnt))
                        {
                            AreaPointList[j].PointList.Add(EndPnt);
                        }
                        //g.DrawLine(new Pen(Color.Red), BaryCnt, EndPnt);
                    }
                }
            }
        ...
        //i为TinEdge的ID号
        private void DrawHullVorEdge(int i,List<PointF> PointList)
        {
            ...
            //else
            //{
            //    g.DrawLine(new Pen(Color.Red, 2), BaryCnt, EndPnt);
            //}
            //三角型轮廓
            PointF[] Points = new PointF[ch.HullPoints.Count];
            Points = ch.HullPoints.ToArray();
            ...
            if (PointPath.IsVisible(BaryCnt) || PointPath.IsVisible(EndPnt))
            {
                PointList.Add(BaryCnt);
                PointList.Add(EndPnt);
            }
            //g.DrawLine(new Pen(Color.Blue, 2), BaryCnt, EndPnt);
        }
