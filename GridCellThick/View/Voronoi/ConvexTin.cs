...
namespace View.Voronoi
{
    public partial class Delaynay
    {
        ...
        public void CreateHull()
        {
            ...
            {
                PointList.Add(new PointF(DS.Vertex[i].x, DS.Vertex[i].y));
            }
            ch.Points = PointList;
            ch.GetConvexHull();
            for (int i = 0; i < ch.HullPoints.Count; i++)
            {
                for (int j = 0; j < DS.VerticesNum; j++)
                {
                    if (ch.HullPoints[i].X == DS.Vertex[j].x && ch.HullPoints[i].Y == DS.Vertex[j].y)
                    {
                        HullPoint.Add(j);
                        break;
                    }
