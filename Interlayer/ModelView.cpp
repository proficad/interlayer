...
bool CModelView::ReadEclipseModel(LPCSTR lpszFileName, LPCSTR lpszModelFileName)
{
	...
	eclipseFile.OpenFile(lpszFileName);
	CSimuToken token;
	...
	do{
		...
		if( token.token_type == CSimuToken::COMMAND )
		{
			switch(commItem.m_nCommID)
			{
			case CEclipseCommandItem::SPECGRID:
				iDatType = 0;
				break;
			case CEclipseCommandItem::DX:
				iDatType = 1;
				break;
			case CEclipseCommandItem::DY:
				iDatType = 2;
				break;
			case CEclipseCommandItem::DZ:
				iDatType = 3;
				break;
			case CEclipseCommandItem::TOPS:
				iDatType = 4;
				break;
			case CEclipseCommandItem::COORD:
				iDatType = 5;
				nCount = 0;
				bcorner = TRUE;
				break;
			case CEclipseCommandItem::ZCORN:
				iDatType = 6;
				break;
			default:
				iDatType = -1;
	...
	if( !bcorner ) 
	{
		int nSize = xSteps.size();
		nSize = ySteps.size();
		if( nSize != (nX*nY*nZ)  )
		{
			::MessageBox(AfxGetMainWnd()->m_hWnd, _T("不是Eclipse数据文件格式!"),_T("提示信息"), MB_OK | MB_ICONWARNING);
			return false;
		}
	...
	{
		double nSize = coordGrid.size()/2.0;
		if( nSize != (nX+1)*(nY+1) )
		{
			::MessageBox(AfxGetMainWnd()->m_hWnd, _T("不是Eclipse数据文件格式!"),_T("提示信息"), MB_OK | MB_ICONWARNING);
			return false;
		}
		nSize = zcornGrid.size();
		if( nSize != (nX*nY*nZ*8) )
		{
			::MessageBox(AfxGetMainWnd()->m_hWnd, _T("不是Eclipse数据文件格式!"),_T("提示信息"), MB_OK | MB_ICONWARNING);
			return false;
	...
	gridModel.m_bCornerPointGrid = bcorner;
	gridModel.m_nGridX = nX;
	gridModel.m_nGridY = nY;
	gridModel.m_nGridZ = nZ;	
	...
	if( !bcorner)
	{
		gridModel.m_gridDX = xSteps;
		gridModel.m_gridDY = ySteps;
		gridModel.m_gridDZ = zValues;
		gridModel.m_gridTops = depthLayer;
	}
	else
	{
		gridModel.m_coordGrid = coordGrid;
		gridModel.m_zcornGrid = zcornGrid;
	}
	...
	for ( int i=0;i<nZ; i++)
	{
		...
		pMF->GetStatusBar().EnablePaneProgressBar (1, i);
	}
