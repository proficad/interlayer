...
{
	double D;
	D = (Row[0][0] * (Row[1][1] * Row[2][2] - Row[2][1] * Row[1][2])) - (Row[0][1] * (Row[1][0] * Row[2][2] - Row[2][0] * Row[1][2])) + (Row[0][2] * (Row[1][0] * Row[2][1] - Row[2][0] * Row[1][1]));
	return D;
}
...
{
	if (Row[1][0] == 0 && Row[2][0] == 0 && Row[2][1] == 0 && Row[0][1] == 0 && Row[0][2] == 0 && Row[1][2] == 0)
		return true;
	return false;
}
...
{
	if (Row[0][0] == Row[1][1] && Row[1][1] == Row[2][2] && Row[0][0] == Row[2][2])
		return true;
	return false;
}
...
{
	if (Row[0][0] == 1 && Row[1][1] == 1 && Row[2][2] == 1 && Row[0][1] == 0 && Row[0][2] == 0 && Row[1][0] == 0 && Row[1][2] == 0 && Row[2][0] == 0 && Row[2][1] == 0)
		return true;
	return false;
}
...
CMatrix33 CMatrix33::Adjoint()
{
	...
	Transpose();
	//A11
	tmpCMatrix33.Row[0][0] = 1 * (Row[1][1] * Row[2][2] - Row[2][1] * Row[1][2]);
	//B12
	tmpCMatrix33.Row[0][1] = -1 * (Row[1][0] * Row[2][2] - Row[2][0] * Row[1][2]);
	//C13
	tmpCMatrix33.Row[0][2] = 1 * (Row[1][0] * Row[2][1] - Row[2][0] * Row[1][1]);
	//A21
	tmpCMatrix33.Row[1][0] = -1 * (Row[0][1] * Row[2][2] - Row[2][1] * Row[0][2]);
	//B22
	tmpCMatrix33.Row[1][1] = 1 * (Row[0][0] * Row[2][2] - Row[2][0] * Row[0][2]);
	//C23
	tmpCMatrix33.Row[1][2] = -1 * (Row[0][0] * Row[2][1] - Row[2][0] * Row[0][1]);
	//A31
	tmpCMatrix33.Row[2][0] = 1 * (Row[0][1] * Row[1][2] - Row[1][1] * Row[0][2]);
	//B32
	tmpCMatrix33.Row[2][1] = -1 * (Row[0][0] * Row[1][2] - Row[1][0] * Row[0][2]);
	//C33
	tmpCMatrix33.Row[2][2] = 1 * (Row[0][0] * Row[1][1] - Row[1][0] * Row[0][1]);
	Transpose();
	return tmpCMatrix33;
}
...
void CMatrix33::SetRotation(const CVector3D& Axis, double t)
{
	...
	double Row1[3] =
	{
		nx* nx* vsin(t) + cos(t), nx* ny* vsin(t) - nz* sin(t), nx* nz* vsin(t) + ny* sin(t)
	};
	double Row2[3] =
	{
		nx* ny* vsin(t) + nz* sin(t), ny* ny* vsin(t) + cos(t), ny* nz* vsin(t) - nx* sin(t)
	};
	double Row3[3] =
	{
		nx* nz* vsin(t) - ny* sin(t), ny* nz* vsin(t) + nx* sin(t), nz* nz* vsin(t) + cos(t)
	};
	CMatrix33 M(Row1, Row2, Row3);
	*this = M;
}

void CMatrix33::SetScale(const CVertex3D& Point)
{
	CVertex3D P = Point;
	double Row1[3] =
	{
...
CVertex3D CMatrix33::GetDiagonal() const
{
	CVertex3D V;
	CMatrix33 M = *this;
	V.SetX(M(0, 0));
