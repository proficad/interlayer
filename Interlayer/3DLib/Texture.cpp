...
int CTexture::Alloc(unsigned int width, unsigned int height, unsigned int depth)
{
	...
	ASSERT(BytePerPixel == 3 || BytePerPixel == 4);
...
int CTexture::ReadFileBMP(char* filename)
{
	...
	END_CATCH TRACE("FileHeader.bfType : %d\n", FileHeader.bfType);
	...
	// 24 bits ?
	if (m_Header.biPlanes != 1 || m_Header.biBitCount != 24)
	{
...
int CTexture::Extract(int left, int top, int right, int bottom)
{
	...
	if (left >= right || top >= bottom)
		return 0;
	if (left < 0 || left >= (int)m_Width || right < 0 || right >= (int)m_Width)
		return 0;
	if (top < 0 || top >= (int)m_Height || bottom < 0 || bottom >= (int)m_Height)
		return 0;
...
int CTexture::PutAlpha(CTexture* pTexture)
{
	...
	ASSERT(size > 0);
	for (int i = 0; i < size; i++)
		m_pData[4 * i + 3] = (unsigned char)(((int)pData[BytePerPixel * i + 0] + (int)pData[BytePerPixel * i + 1] + (int)pData[BytePerPixel * i + 2]) / 3);
...
// Draw
//********************************************
int CTexture::Draw(CDC* pDC, int xOffset, int yOffset, int width, int height)
{
	// Checking
	if (!IsValid())
		return 0;
	...
	// Painting
	return SetDIBitsToDevice(pDC->m_hDC, xOffset, yOffset, width, height, 0, 0, 0, m_Height, GetData(), (CONST BITMAPINFO *)&m_Header, DIB_RGB_COLORS);
}

//********************************************
// ReadBuffer
//********************************************
int CTexture::ReadBuffer(unsigned char* buffer, int width, int height, int depth)
{
	if (buffer == NULL)
		return 0;
...
// ReadBuffer
//********************************************
int CTexture::ReadBuffer(float* buffer, int width, int height, int depth)
{
	if (buffer == NULL)
		return 0;
...
int CTexture::Grey(unsigned int x, unsigned int y)
{
	...
	if (BytePerPixel == 1)
		...
		return (int)((int)m_pData[m_WidthByte32 * y + x * BytePerPixel + 0] + (int)m_pData[m_WidthByte32 * y + x * BytePerPixel + 1] + (int)m_pData[m_WidthByte32 * y + x * BytePerPixel + 2]) / 3;
}

//********************************************
// Color
//********************************************
void CTexture::Color(unsigned int x, unsigned int y, unsigned char* pRed, unsigned char* pGreen, unsigned char* pBlue)
{
	ASSERT(x < m_Width && x >= 0);
	ASSERT(y < m_Height && y >= 0);
	int BytePerPixel = m_Depth / 8;
