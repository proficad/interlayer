...
DWORD CRefObject::AddRef()
{ 
	return ++m_dwRefCount; 
}

// Returns the current reference count.
DWORD CRefObject::GetRefCount() const
{ 
	return m_dwRefCount;
}
...
//
IMPLEMENT_SERIAL(CRefObList, CObject, VERSIONABLE_SCHEMA|0)
...
void CRefObList::Serialize(CArchive& ar)
{
	...
	else
	{
		...
		{
			ar >> pOb;
			AddTail((CRefObject*)pOb);
		}
	}
}

int CRefObList::GetCount() const
{ 
	return m_nCount;
}

BOOL CRefObList::IsEmpty() const
{
	return m_nCount == 0;
}

CRefObject*& CRefObList::GetHead()
{ 
	ASSERT(m_pNodeHead != NULL);
	return m_pNodeHead->data; 
}

CRefObject* CRefObList::GetHead() const
{ 
	ASSERT(m_pNodeHead != NULL);
	return m_pNodeHead->data; 
}

CRefObject*& CRefObList::GetTail()
{ 
	ASSERT(m_pNodeTail != NULL);
	return m_pNodeTail->data; 
}

CRefObject* CRefObList::GetTail() const
{ 
	ASSERT(m_pNodeTail != NULL);
	return m_pNodeTail->data; 
}

POSITION CRefObList::GetHeadPosition() const
{ 
	return (POSITION) m_pNodeHead; 
}

POSITION CRefObList::GetTailPosition() const
{ 
	return (POSITION) m_pNodeTail; 
}

CRefObject*& CRefObList::GetNext(POSITION& rPosition) // return *Position++
{ 
...
CRefObject* CRefObList::GetNext(POSITION& rPosition) const // return *Position++
{
	CNode* pNode = (CNode*) rPosition;
	ASSERT(AfxIsValidAddress(pNode, sizeof(CNode)));
	rPosition = (POSITION) pNode->pNext;
...
CRefObject*& CRefObList::GetPrev(POSITION& rPosition) // return *Position--
{ 
	CNode* pNode = (CNode*) rPosition;
	ASSERT(AfxIsValidAddress(pNode, sizeof(CNode)));
...
CRefObject* CRefObList::GetPrev(POSITION& rPosition) const // return *Position--
{
	CNode* pNode = (CNode*) rPosition;
	ASSERT(AfxIsValidAddress(pNode, sizeof(CNode)));
	rPosition = (POSITION) pNode->pPrev;
...
CRefObject*& CRefObList::GetAt(POSITION position)
{ 
...
CRefObject* CRefObList::GetAt(POSITION position) const
{ 
	CNode* pNode = (CNode*) position;
	ASSERT(AfxIsValidAddress(pNode, sizeof(CNode)));
	return pNode->data;
