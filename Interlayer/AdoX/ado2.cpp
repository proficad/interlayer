...
BOOL CADODatabase::Open(LPCTSTR lpstrConnection, LPCTSTR lpstrUserID, LPCTSTR lpstrPassword)
{
	...
	try
	{
		...
		hr = m_pConnection->Open(
			...
			_bstr_t(lpstrUserID), 
			_bstr_t(lpstrPassword), 
			ADODB::adConnectUnspecified // 同步打开连接
			);

		// adAsyncConnect 	异步打开连接, 可以调用事件
...
{	
	...
	try
	{
  		if(bIsSelect || nOption == openQuery || nOption == openUnknown)
  			m_pRecordset->Open((LPCSTR)m_strQuery, mpdb.GetInterfacePtr(), 
...
BOOL CADORecordset::GetFieldValue(LPCTSTR lpFieldName, CString& strValue, CString strFormat)
{
	CString str;
	_variant_t vtFld;
	_bstr_t bstrBuff;
	...
	try
	{
		...
		case VT_R4:
			...
			{
				bstrBuff = (_bstr_t)m_pRecordset->Fields->GetItem(lpFieldName)->Value;
				str = (LPCSTR)bstrBuff;
			}
			else
				str.Format(strFormat,vtFld.fltVal);
			break;
		case VT_R8:
			if(strFormat.IsEmpty())
			{
				...
				str.Format(strFormat,vtFld.dblVal);
			break;
		case VT_BSTR:
			str = vtFld.bstrVal;
			break;
		...
		case VT_UI1:
			if(strFormat.IsEmpty())
			{
				...
				str.Format(strFormat,vtFld.iVal);			
			break;
		case VT_INT:
			if(strFormat.IsEmpty())
			{
				...
				str.Format(strFormat,vtFld.intVal);
			break;
		case VT_I4:
			if(strFormat.IsEmpty())
			{
				...
				str.Format(strFormat,vtFld.lVal);
			break;
		case VT_UI4:
			if(strFormat.IsEmpty())
			{
				...
				str.Format(strFormat,vtFld.ulVal);
			break;
		case VT_DECIMAL:
			{				
				if(strFormat.IsEmpty())
...
BOOL CADORecordset::GetFieldValue(int nIndex, CString& strValue, CString strFormat)
{
	CString str;
	_variant_t vtFld;
	_variant_t vtIndex;
	...
	{		
		...
		case VT_R4:
			if(strFormat.IsEmpty())
			{
				bstrBuff = (_bstr_t)m_pRecordset->Fields->GetItem(vtIndex)->Value;
				str = (LPCSTR)bstrBuff;
			}
			else
				str.Format(strFormat,vtFld.fltVal);
...
BOOL CADORecordset::SetPageSize(int nSize)
{
	try
	{
...
BOOL CADORecordset::GetColumns(LPCTSTR lpstrTable)
{
	CString sTableName = lpstrTable;
	...
	try
	{
		...
		m_pRecordset = m_pConnection->OpenSchema(ADODB::adSchemaColumns, varData, vtSchemaID);
		return TRUE;
	}
	catch(_com_error &e)
	{
