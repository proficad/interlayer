...
class AFX_EXT_CLASS CJetEngine
{
public:
...
public:
	...
	BOOL IsOpen();
	void Close();
	void SetConnectionMode(ADODB::ConnectModeEnum nMode)
...
public:
	...
	BOOL GetColumns(LPCTSTR lpstrTable);
	long GetFieldCount()
	...
	BOOL GetFieldValue(LPCTSTR lpFieldName, CString& strValue, CString strFormat = _T(""));
	BOOL GetFieldValue(int nIndex, CString& strValue, CString strFormat = _T(""));
...
*/
	CADOParameter(int nType, long lSize = 0, int nDirection = ADODB::adParamInput, CString strName = _T(""));
...
public:
	...
	CADOCommand(CADODatabase* pAdoDatabase, CString strCommandText = _T(""), int nCommandType = ADODB::adCmdStoredProc);
	...
	BOOL Execute(int nCommandType = ADODB::adCmdStoredProc);
	int GetRecordsAffected()
		{return m_nRecordsAffected;};
...
class AFX_EXT_CLASS CADOException
{
public:
	CADOException() :
	  m_lErrorCode(0),
	  ...
	  {return m_strError;};
	  void SetErrorMessage(LPCSTR lpstrError = NULL)
