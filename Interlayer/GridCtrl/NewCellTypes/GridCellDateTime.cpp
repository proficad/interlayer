...
IMPLEMENT_SERIAL(CGridCellDateTime, CGridCell, VERSIONABLE_SCHEMA | CURVERSION )
...
void CGridCellDateTime::SetTime(CTime time)
{
	...
	else if (DTS_LONGDATEFORMAT == m_dwStyle || DTS_SHORTDATEFORMAT == m_dwStyle) 
	{
#ifdef _WIN32_WCE
		CString strTemp;
		strTemp.Format(_T("%02d/%02d/%02d"), 
		...
		SetText(strTemp);
#else
		SetText(m_cTime.Format(("%Y年%m月%d日")));
#endif
	}
}
...
void CInPlaceDateTime::OnKillFocus(CWnd* pNewWnd) 
{
	...
	if( GetOwner()->GetSafeHwnd() != pNewWnd->GetSafeHwnd() )
	{
		return;
	}
...
void CGridCellDateTime::Serialize(CArchive &ar)
{
	...
	ar.SerializeClass(RUNTIME_CLASS(CGridCellDateTime));
	if( ar.IsLoading() )	
	{
