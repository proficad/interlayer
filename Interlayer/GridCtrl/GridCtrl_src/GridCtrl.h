...
typedef struct _GRID_API tagNM_GRIDVIEW {
    NMHDR hdr;
    int   iRow;
    int   iColumn;
} NM_GRIDVIEW;
...
typedef struct _GRID_API tagGV_DISPINFO {
    NMHDR   hdr;
    GV_ITEM item;
} GV_DISPINFO;

// This is sent to the Grid from child in-place edit controls
typedef struct _GRID_API tagGV_CACHEHINT {
    NMHDR      hdr;
    CCellRange range;
} GV_CACHEHINT;
...
class _GRID_API CGridCtrl : public CWnd
{
...
public:
	...
	BOOL SetHeaderRowCount(int nHeaderRows)		{ m_nHeaderRows = nHeaderRows; return TRUE;}
	BOOL SetHeaderColumnCount(int nHeaderCols) { m_nHeaderCols = nHeaderCols; return TRUE;}
	int  GetHeaderRowCount() const              { return m_nHeaderRows; }
    int  GetHeaderColumnCount() const           { return m_nHeaderCols; }
	// ########################### end
	...
	//@**#---2003-01-21 15:15:57 (张军(ZhangJun))---#**@
	int  GetRowAddEnable(int nRow) const;
    BOOL SetRowAddEnable(int row, int nEnable);
    int  GetColAddEnable(int nCol) const;
    BOOL SetColAddEnable(int col, int nEnable);
    ...
    int  GetHeaderColumnWidth() const;

	void SetReSize(BOOL bValue) { m_bReSize = bValue;}
	BOOL GetReSize() { return m_bReSize ; }
    //###################### end 

	// zj 2005.1.9 Add ################## begin
	void SetReport(BOOL bValue) { m_bReport = bValue;}
	BOOL GetReport() { return m_bReport ; }
    //###################### end
...
///////////////////////////////////////////////////////////////////////////////////
public:
    int  InsertColumn(LPCTSTR strHeading, int nColumn = -1, BOOL bBefore = TRUE);
    int  InsertRow(LPCTSTR strHeading, int nRow = -1, BOOL bBefore = TRUE);
	...
	//////////////////////////////////////////////////////////////////////////
	// 张军增加
	BOOL IsColumnSelected(int nCol) const;	
	BOOL IsRowSelected(int nRow) const;
	double GetMaxDatColumn(int nCol, double dMin = 9998.0 );
	double GetMinDatColumn(int nCol, double dMax = -9000.0);
	double GetMaxDatRow(int nRow, double dMin = 9998.0);
	double GetMinDatRow(int nRow, double dMax = -9000.0);
	//////////////////////////////////////////////////////////////////////////
...
#ifndef _WIN32_WCE
	// zj 2004.6.21 ADD
	BOOL SaveToExcelFile(LPCTSTR filename);
	//*********************//
...
public:
	...
	void SetPrintMarginInfo(float nHeaderHeight, float nFooterHeight,
        float nLeftMargin, float nRightMargin, float nTopMargin,
        float nBottomMargin, float nGap);

    void GetPrintMarginInfo(float &nHeaderHeight, float &nFooterHeight,
        float &nLeftMargin, float &nRightMargin, float &nTopMargin,
        float &nBottomMargin, float &nGap);
	//############################
...
protected:
	...
	virtual void PrintFixedRowCells(int nStartColumn, int nStopColumn, int& row, CRect& rect,
                                    CDC *pDC, BOOL& bFirst);
	virtual void PrintHeaderRowCells(int nStartColumn, int nStopColumn, int& row, CRect& rect,
                                    CDC *pDC, BOOL& bFirst);
    virtual void PrintColumnHeadings(CDC *pDC, CPrintInfo *pInfo);
    virtual void PrintHeader(CDC *pDC, CPrintInfo *pInfo);
    virtual void PrintFooter(CDC *pDC, CPrintInfo *pInfo);
    virtual void PrintRowButtons(CDC *pDC, CPrintInfo* /*pInfo*/);
	virtual void PrintRowHeadings(CDC *pDC, CPrintInfo* /*pInfo*/);
#endif
...
protected:
	...
	BOOL		m_bReSize;//
	BOOL        m_bReport;//报表模式
	BOOL		m_bPrintFixedRow; // 是否打印固定行
	BOOL		m_bPrintFixedCol; // 是否打印固定列
	BOOL		m_bAutoAddRow;	  // 是否自动加行
	BOOL		m_bAutoAddCol;	  // 是否自动加列

    // Cell size details
    int         m_nRows, m_nFixedRows, m_nCols, m_nFixedCols;

	//zj 2002.12.18 Add ################ begin
	int			m_nHeaderRows, m_nHeaderCols;//上标题行数， 左标题列数
	CUIntArray  m_arRowAddEnable, m_arColAddEnable;
	// ###################### end
...
public:	
	void SetPrintFixedRow(const BOOL &bValue = TRUE);
	BOOL GetPrintFixedRow();
	void SetPrintFixedCol(const BOOL &bValue = TRUE);
	BOOL GetPrintFixedCol();

	void SetAutoAddRow(const BOOL &bValue = TRUE);
	BOOL GetAutoAddRow();
	void SetAutoAddCol(const BOOL &bValue = TRUE);
	BOOL GetAutoAddCol();

	virtual void Serialize(CArchive &ar);
	void AnalyPrintFormat(CString &Format, CString &left, CString &center, CString &right, CPrintInfo *pInfo);
...
*/
	...
	//  zj 2002.12.18 modified ################
	// 用户定义打印区域不用行用毫米表示
	float       m_nHeaderHeight, m_nFooterHeight, m_nLeftMargin,
                m_nRightMargin, m_nTopMargin, m_nBottomMargin, m_nGap;

	BOOL		m_bPrintHeader, m_bPrintHeaderLine; //是否打印页眉及页脚
	BOOL		m_bPrintFooter, m_bPrintFooterLine;

	CString		m_strHeaderFormat;
	CString		m_strFooterFormat;
