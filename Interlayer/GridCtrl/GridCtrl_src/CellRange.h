...
public:
    ...
    int  Count()
	{
		if ( m_nMaxRow == -1 || m_nMaxCol==-1 || m_nMinRow == -1 || m_nMinCol == -1 ) 
			return 0;
	...
	void operator+=(const CCellRange& rhs);
    void offset(const int left, const int top);
...
inline void CCellRange::offset(const int left, const int top)
{
    m_nMinRow += top;
	m_nMinCol += left;
}
