...
void CTitleTip::Show(CRect rectTitle, LPCTSTR lpszTitleText, UINT nFormat, int xoffset /*=0*/,
                     ...
                     COLORREF crBackClr /* CLR_DEFAULT */)
{
	...
	dc.DrawText(strTitle, rt, DT_CALCRECT);
	...
	{
		rectDisplay.right = rectDisplay.left + rt.Width() + xoffset;
		if( rectDisplay.bottom < ( rectDisplay.top + rt.Height() ))
			rectDisplay.bottom = rectDisplay.top + rt.Height();	
	}
    ...
    if ( rectDisplay.right > rectTitle.right-xoffset )
    {
		...
		dc.DrawText(strTitle, rect, nFormat);
