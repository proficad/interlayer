...
void CALLBACK DrawDotLine(int x, int y, struct str_DrawDotLine * pDr) 
{ 
	if( pDr->bFlag )
	{
		::SetPixel(pDr->hDC,x,y,pDr->clr);
		pDr->bFlag = FALSE;
	}
	else
		pDr->bFlag = TRUE;
...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
	...
	m_nHeaderRows		  = 0; //zj 2002.12.18 Add
	m_nHeaderCols		  = 0; //zj 2002.12.18 Add
    ...
    m_bWysiwygPrinting    = TRUE;      // use size-to-width printing
    ...
    SetPrintMarginInfo(5, 5, 20, 20, 20, 20, 5);//都为1cm
#endif
	m_bPrintHeader = FALSE;
	m_bPrintHeaderLine = FALSE;
	m_bPrintFooter = FALSE;
	m_bPrintFooterLine = FALSE;

	m_strHeaderFormat = _T("&c&f&大庆拓普石油科技开发有限公司");
	m_strFooterFormat = _T("&l共&t页 第&p页&r打印日期：%Y年%m月%d日 %H:%M:%S");	
	...
	m_bPrintFixedRow = FALSE; // 是否打印固定行
	m_bPrintFixedCol = FALSE; // 是否打印固定列
	m_bAutoAddRow	= FALSE;
	m_bAutoAddCol	= FALSE;
	//////////////////////////////////////////////////////////////////////////
...
//#endif
	...
	//@**#---2002-11-26 10:55:01 (张军(ZhangJun))---#**@
//if (::IsWindow(m_hWnd))
...
BOOL CGridCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwStyle)
{
    ...
    TRY
    {
		...
		m_arRowAddEnable.SetSize(m_nRows);  //初始是否可以加行列数据大小
		m_arColAddEnable.SetSize(m_nCols);
    }
...
////////////////////////////////////////////////////////////////////////// 
   if( pParentWnd->IsKindOf(RUNTIME_CLASS(CView)))
	   m_pParentView = (CView*)pParentWnd;
...
{	
    ...
    else if (nChar == VK_DOWN)
    {
		...
		if( m_bAutoAddRow && next.row >= GetRowCount() )
		{
			InsertRow("");
			bFoundVisible = TRUE;
			Invalidate();
		...
		{
            next.row = iOrig;
			GetParent()->SendMessage(WM_KEYDOWN, VK_DOWN, 0);
			return;
		}
    }
    else if (nChar == VK_UP)
    ...
    else if (nChar == VK_RIGHT || (nChar == VK_TAB && !IsSHIFTpressed()) )
    {
		...
		if( m_bAutoAddCol && next.col >= GetColumnCount() )
		{
			InsertColumn("");
			bFoundVisible = TRUE;
			Invalidate();
...
//            pDC->LineTo(x-1, VisRect.bottom);
			...
			struct str_DrawDotLine dr;
			dr.hDC = pDC->m_hDC;
			dr.clr = m_crGridLineColour;
			dr.bFlag = TRUE;
			LineDDA(x-1, nFixedRowHeight, x-1,VisRect.bottom, (LINEDDAPROC)DrawDotLine, (LPARAM)&dr);
...
//            pDC->LineTo(VisRect.right,  y-1);
			...
			dr.clr = m_crGridLineColour;
			dr.bFlag = TRUE;
			LineDDA(nFixedColWidth, y-1, VisRect.right, y-1,(LINEDDAPROC)DrawDotLine, (LPARAM)&dr);
        }
    }
...
//				dr.clr = m_crGridLineColour;
//				dr.bFlag = TRUE;
//				LineDDA(rect.left, rect.bottom, rect.right + 1,rect.bottom, (LINEDDAPROC)DrawDotLine, (LPARAM)&dr);
            }
            if (m_nGridLines == GVL_BOTH || m_nGridLines == GVL_VERT)
            {
								...
								BOOL bSelectPastedCells /*=TRUE*/)
{
    ...
    do
    {
		...
		if( m_bAutoAddRow && iRowVis >= GetRowCount() )
			InsertRow("");
        ...
        while (!strLine.IsEmpty())
        {
			...
			if( m_bAutoAddCol && !IsValid(TargetCell) )
				InsertColumn("");
...
{
    if (point.x >= GetFixedColumnWidth() && !m_bReSize)
        return FALSE;
...
{
    if (point.y >= GetFixedRowHeight() && !m_bReSize )
        return FALSE;
...
//////////////////////////////////////////////////////////////////////////
// 张军增加 
BOOL CGridCtrl::IsColumnSelected(int nCol) const
{
...
BOOL CGridCtrl::IsRowSelected(int nRow) const
{
...
double CGridCtrl::GetMaxDatColumn(int nCol, double dMin)
{
	...
	double dVal;
	CString strTmp;
	for (int i = nFixedRows; i < nRows; i++ )
	{
		strTmp = GetItemText(i, nCol);
		dVal = atof(strTmp);
		if(dVal >= dMin)
			continue;
...
double CGridCtrl::GetMinDatColumn(int nCol, double dMax)
{
	int nRows = GetRowCount();
	int nFixedRows = GetFixedRowCount();
	if(nCol >= GetColumnCount() )
...
double CGridCtrl::GetMaxDatRow(int nRow, double dMin)
{
	int nCols = GetColumnCount();
	int nFixedCols = GetFixedColumnCount();
	if(nRow >= GetRowCount() )
		return 0.0;
	...
	double dVal;
	CString strTmp;
	for (int i = nFixedCols; i < nCols; i++ )
	{
		strTmp = GetItemText(nRow, i);
		dVal = atof(strTmp);
		if(dVal >= dMin)
			continue;
...
double CGridCtrl::GetMinDatRow(int nRow, double dMax)
{
	int nCols = GetColumnCount();
	int nFixedCols = GetFixedColumnCount();
	if(nRow >= GetRowCount() )
...
BOOL CGridCtrl::SetRowCount(int nRows)
{
    ...
    TRY
    {
		...
		m_arRowAddEnable.SetSize(nRows);		
...
BOOL CGridCtrl::SetColumnCount(int nCols)
{
    ...
    TRY 
    {
		...
		m_arColAddEnable.SetSize(nCols);
...
int CGridCtrl::InsertColumn(LPCTSTR strHeading, int nColumn  /* = -1 */, BOOL bBefore)
{
	...
	if (!GetVirtualMode())
	{
		...
		{
			for( int col = m_nCols - 1; col > start; col --)
			{
				pCell = GetCell(row, col);
				if( pCell->IsMerged() )
				{
				...
				if( pCell->IsMergeWithOthers() )
				{
					CCellID cellID = pCell->GetMergeCellID();
					if( cellID.col > start ) //只有插入列之后的附属单元格才往后移
					{
						cellID.offset(0, 1);
						pCell->SetMergeCellID(cellID);
					}					
				}
			}
		}
    ...
    TRY
    {
        ...
        else
        {
			if( bBefore ) //修改
			{
				...
				m_arColAddEnable.InsertAt(nColumn,(UINT)0);
				if (!GetVirtualMode())
				{
					for (int row = 0; row < m_nRows; row++) 
					{
			...
			{
				m_arColWidths.InsertAt(nColumn+1, (UINT)0);
				m_arColAddEnable.InsertAt(nColumn+1,(UINT)0);
				if (!GetVirtualMode())
				{
					for (int row = 0; row < m_nRows; row++) 
					{
						...
						if (!pRow)
							return -1;
						pRow->InsertAt(nColumn+1, CreateCell(row, nColumn+1));
					}
				}
	...
	m_arColWidths[nColumn] = GetDefCellWidth();
    
    if (m_idCurrentCell.col != -1 && nColumn < m_idCurrentCell.col && bBefore)
        m_idCurrentCell.col++;
		...
		for(int row = 0 ; row < m_nRows; row ++)
		{
			...
			{	
				...
				if( bBefore )
				{
					...
					tmp.Set();
					pCell->SetMergeRange(tmp);
					CCellID cell(row, start-1);
					pCell->SetMergeCellID(cell);
					
					p = GetCell(cell);
					tmp.Set(0,0,0,1);
					range += tmp;
					p->SetMergeRange(range);
					p->SetText(str);
				}
				else
				{
					...
					p = GetCell(row, start+1);
					CCellID cell(row, start);
					p->SetMergeCellID(cell);
				}
			...
			if( pCell != NULL && pCell->IsMergeWithOthers() )
			{
				CCellID cellID = pCell->GetMergeCellID();
				CGridCellBase *p;
				if(  cellM != cellID && cellID.col != (bBefore?nColumn:nColumn-1) )
				{
					cellM = cellID;
					p = GetCell(cellID);
					CCellRange range = p->GetMergeRange();
					CCellRange tmp;
					tmp.Set(0,0,0,1);
					range += tmp;
					p->SetMergeRange(range);
	...
	// **加属性
	//@**#---2003-01-22 10:50:44 (张军(ZhangJun))---#**@
	if (!GetVirtualMode())
    {
		CGridCellBase *pCell;
		CRuntimeClass * pClass;
		CGridCellBase *pInsertCell;
		
        for (int row = 0; row < m_nRows; row++)
        {
			if( bBefore )
			{
				pCell = GetCell(row,nColumn+1);
				if( pCell != NULL ) 
					m_arColAddEnable[nColumn] = m_arColAddEnable[nColumn+1];
			}
			else
			{
				pCell = GetCell(row,nColumn-1);
			...
			if( pCell != NULL )
			{
				pClass = pCell->GetRuntimeClass();
				SetCellType(row, nColumn, pClass);
				
				pInsertCell = GetCell(row, nColumn);
				
				pInsertCell->SetState(pCell->GetState());
				pInsertCell->SetFormat(pCell->GetFormat());
				pInsertCell->SetTextClr(pCell->GetTextClr());
				pInsertCell->SetBackClr(pCell->GetBackClr());
				pInsertCell->SetFont(pCell->IsDefaultFont()? NULL : pCell->GetFont());
				pInsertCell->Show(pCell->IsShow());
				
				//zj 2004.6.23 add
				pInsertCell->SetCellBorderLine(pCell->GetCellBorderLine());
				
				//**********设置单元格类型
				DWORD nType = pCell->m_nType;			
				
				switch( nType )				
				{
				case 1:
					{
						CGridCellDateTime * p1 = (CGridCellDateTime*) pCell;
						CGridCellDateTime * p2 = (CGridCellDateTime*) pInsertCell;
						
						p2->m_dwStyle = p1->m_dwStyle;						
					}
					break;
				case 3:
					{
						CGridCellCheckCombo * p1 = (CGridCellCheckCombo*) pCell;
						CGridCellCheckCombo * p2 = (CGridCellCheckCombo*) pInsertCell;
						
						CStringArray *pAr= p1->GetOptions();
						int num = pAr->GetSize();
						CString str;
						CStringArray ar;
						for (int i = 0; i < num; i++)
				...
				case 4:
					{				
						CGridCellCombo * p1 = (CGridCellCombo*) pCell;
						CGridCellCombo * p2 = (CGridCellCombo*) pInsertCell;
				...
				case 5:
					{
						CGridCellNumeric * p1 = (CGridCellNumeric*) pCell;
						CGridCellNumeric * p2 = (CGridCellNumeric*) pInsertCell;
						
						p2->m_bSigned = p1->m_bSigned;						
					}
					break;
				case 6:
					{
						CGridCellDouble * p1 = (CGridCellDouble*) pCell;
						CGridCellDouble * p2 = (CGridCellDouble*) pInsertCell;
						
						p2->m_bSigned = p1->m_bSigned;
						p2->m_nEditNumericLeftDigits = p1->m_nEditNumericLeftDigits;
						p2->m_nEditNumericRightDigits = p1->m_nEditNumericRightDigits;						
					}
					break;
				case 7:
					{
						CGridCellCurrency * p1 = (CGridCellCurrency*) pCell;
						CGridCellCurrency * p2 = (CGridCellCurrency*) pInsertCell;
						
						p2->m_nDigitsInGroup = p1->m_nDigitsInGroup;
						p2->m_nMaxDecimalPlaces = p1->m_nMaxDecimalPlaces;
						p2->m_nMaxWholeDigits = p1->m_nMaxWholeDigits;						
					}
					break;
				case 8:
					{
						CGridCellMask * p1 = (CGridCellMask*) pCell;
						CGridCellMask * p2 = (CGridCellMask*) pInsertCell;
						
						p2->m_strEditMaskedMask = p1->m_strEditMaskedMask;
					}
					break;				
				default:
					break;
	...
	SendMessageToParent(-1, nColumn, GVN_INSERTITEM);

    return nColumn;
}

// Insert a row at a given position, or add to end of rows (if nRow = -1)
int CGridCtrl::InsertRow(LPCTSTR strHeading, int nRow /* = -1 */, BOOL bBefore)
{
	...
	if (!GetVirtualMode())
	{
		...
		for( int col = 0; col < m_nCols; col ++)
		{
			for(int row = m_nRows - 1 ; row > start; row --)
			{			
				pCell = GetCell(row, col);
				if( pCell->IsMerged() )
				{
				...
				{
					CCellID cellID = pCell->GetMergeCellID();
					if( cellID.row > start ) //只有插入行之后的附属单元格才往后移
					{
						cellID.offset(1, 0);
						pCell->SetMergeCellID(cellID);
					}					
    ...
    TRY
    {
        ...
        else
        {
			if( bBefore )//修改
			{
				...
				m_arRowAddEnable.InsertAt(nRow, (UINT)0);
				if (!GetVirtualMode())
					m_RowData.InsertAt(nRow, new GRID_ROW);
			}
			else
			{
				m_arRowHeights.InsertAt(nRow+1, (UINT)0);
				m_arRowAddEnable.InsertAt(nRow+1, (UINT)0);
				if (!GetVirtualMode())
					m_RowData.InsertAt(nRow+1, new GRID_ROW);
				nRow++;
			}
        }
    ...
    if (m_idCurrentCell.row != -1 && nRow < m_idCurrentCell.row && bBefore)
        m_idCurrentCell.row++;
		...
		for(int col = 0 ; col < m_nCols; col ++)
		{
			...
			{	
				...
				else
				{
					...
					p = GetCell(start+1, col);
					CCellID cell(start, col);
					p->SetMergeCellID(cell);
				}
			...
			if( pCell->IsMergeWithOthers() )
			{
				CCellID cellID = pCell->GetMergeCellID();
				CGridCellBase *p;
				if(  cellM != cellID && cellID.row != (bBefore?nRow:nRow-1))
				{
					cellM = cellID;
					p = GetCell(cellID);
					CCellRange range = p->GetMergeRange();
	...
	if (!GetVirtualMode())
    {
        ...
        for (int col = 0; col < m_nCols; col++)
        {
			if( bBefore )
			{
				pCell = GetCell(nRow+1,col);
				if( !pCell)
					break;
				m_arRowAddEnable[nRow] = m_arRowAddEnable[nRow+1];
			}
			else
			{
				pCell = GetCell(nRow-1,col);
			...
			pClass = pCell->GetRuntimeClass();
			SetCellType(nRow, col, pClass);
			
			pInsertCell = GetCell(nRow, col);
	...
	SendMessageToParent(nRow, -1, GVN_INSERTITEM);
...
BOOL CGridCtrl::DeleteColumn(int nColumn)
{
			...
			{		
				...
				if( pCell->IsMerged() )
				{
					...
					if( col == start )
					{
						tmp.Set(0, 0, 0, -1);
						range += tmp;
						if( range.Count() > 0 ) //不是一列
						{
							CString str = pCell->GetText();
							CGridCellBase *p = GetCell(row, col + 1 );
							p->SetMergeRange(range);
							p->SetText(str);
						}
					}
				...
				if( pCell->IsMergeWithOthers() )
				{
					CCellID cellID = pCell->GetMergeCellID();
					CGridCellBase *p;
					if(  cellM != cellID && cellID.col < start )
					{
						cellM = cellID;
						p = GetCell(cellID);
						CCellRange range = p->GetMergeRange();
	...
	m_arColAddEnable.RemoveAt(nColumn);
    m_nCols--;
	...
	SendMessageToParent(-1, nColumn, GVN_DELETEITEM);
...
BOOL CGridCtrl::DeleteRow(int nRow)
{
			...
			{		
				...
				if( pCell->IsMerged() )
				{
					...
					if( col == start )
					{
						...
						{
							CString str = pCell->GetText();
							CGridCellBase *p = GetCell(row+1, col);
							p->SetMergeRange(range);
							p->SetText(str);
						}
					}
				...
				if( pCell->IsMergeWithOthers() )
				{
					CCellID cellID = pCell->GetMergeCellID();
					CGridCellBase *p;
					if(  cellM != cellID && cellID.row < start )
					{
						cellM = cellID;
						p = GetCell(cellID);
						CCellRange range = p->GetMergeRange();
	...
	m_arRowAddEnable.RemoveAt(nRow);
	...
	SendMessageToParent(nRow, -1, GVN_DELETEITEM);
...
BOOL CGridCtrl::DeleteAllItems()
{
	...
	m_arColAddEnable.RemoveAll();
	m_arRowAddEnable.RemoveAll();
	...
	SendMessageToParent(-1, -1, GVN_DELETEITEM);
...
//@**#---2003-01-21 15:19:06 (张军(ZhangJun))---#**@
//

int CGridCtrl::GetRowAddEnable(int nRow) const
{
    ASSERT(nRow >= 0 && nRow < m_nRows);
    if (nRow < 0 || nRow >= m_nRows)
        return -1;

    return m_arRowAddEnable[nRow];
}

int CGridCtrl::GetColAddEnable(int nCol) const
{
    ASSERT(nCol >= 0 && nCol < m_nCols);
    if (nCol < 0 || nCol >= m_nCols)
        return -1;

    return m_arColAddEnable[nCol];
}

BOOL CGridCtrl::SetRowAddEnable(int nRow, int nEnable)
{
    ASSERT(nRow >= 0 && nRow < m_nRows);
    if (nRow < 0 || nRow >= m_nRows)
        return FALSE;

    m_arRowAddEnable[nRow] = nEnable;
    
    return TRUE;
}

BOOL CGridCtrl::SetColAddEnable(int nCol, int nEnable)
{
    ASSERT(nCol >= 0 && nCol < m_nCols);
    if (nCol < 0 || nCol >= m_nCols)
        return FALSE;

    m_arColAddEnable[nCol] = nEnable;
    
    return TRUE;
}

//
//@**#---2003-01-21 15:19:14 (张军(ZhangJun))---#**@
...
// zj 2002.12.18 add ###################### begin

int CGridCtrl::GetHeaderRowHeight() const
{
    int nHeight = 0;
	int nFixedRows = GetFixedRowCount();
    for (int i = 0; i < m_nHeaderRows; i++)
        nHeight += GetRowHeight(i + nFixedRows);

    return nHeight;
}

int CGridCtrl::GetHeaderColumnWidth() const
{
    int nWidth = 0;
	int nFixedCols = GetFixedColumnCount();
    for (int i = 0; i < m_nFixedCols; i++)
        nWidth += GetColumnWidth(i+nFixedCols);
            ...
            if (m_bTitleTips)
            {
                ...
                if (pCell)
                {
                        ...
                        && GetCellRect(idCurrentCell.row, idCurrentCell.col, CellRect) )
                    {
						...
						m_TitleTip.Show(TextRect, pCell->GetTipText(), pCell->GetFormat(), 0, CellRect,
...
void CGridCtrl::SetPrintMarginInfo(float nHeaderHeight, float nFooterHeight,
    float nLeftMargin, float nRightMargin, float nTopMargin,
    float nBottomMargin, float nGap)
{
...
void CGridCtrl::GetPrintMarginInfo(float &nHeaderHeight, float &nFooterHeight,
    float &nLeftMargin, float &nRightMargin, float &nTopMargin,
    float &nBottomMargin, float &nGap)
{
...
//	//zj 2002.12.18 modified
	LOGFONT lgFont;
	GetFont()->GetLogFont(&lgFont);
	m_PrinterFont.CreateFontIndirect(&lgFont);	

	int nMargins = (m_nLeftMargin+m_nRightMargin) / 25.4 * ScreenPixelsPerInch.cx; 
	// 化为像素
	// zj 2002.12.18 modified 
    ...
    {
        m_LogicalPageSize.cx = ScreenPixelsPerInch.cx * m_PaperSize.cx / PaperPixelsPerInch.cx;// * 3 / 4;  //zj 2002.12.18 modified
        m_LogicalPageSize.cy = ScreenPixelsPerInch.cy * m_PaperSize.cy / PaperPixelsPerInch.cy;// * 3 / 4;  //zj 2002.12.18 modified
    }
    else
    {
	...
	// zj 2002.12.18 modified ####################### begin

	m_nPageHeight = m_LogicalPageSize.cy - 
		( m_nHeaderHeight + m_nFooterHeight + m_nTopMargin + m_nBottomMargin + 2*m_nGap) / 25.4 * ScreenPixelsPerInch.cy;

    if(!GetReport())
		m_nPageHeight -= GetHeaderRowHeight();
	else
		m_nPageHeight -= GetFixedRowHeight();
                     

    // Get the number of pages. Assumes no row is bigger than the page size.
    int nTotalRowHeight = 0;
    m_nNumPages = 1;
    for (int row = GetFixedRowCount() + GetHeaderRowCount(); row < GetRowCount(); row++)
    {
	...
	int nFixedCols = GetFixedColumnCount();
	if(!GetReport())
    for( i1=0; i1 < GetHeaderColumnCount(); i1++)
    {
        iColumnOffset += GetColumnWidth( i1 + nFixedCols );
    }
	else
    ...
    if( m_bWysiwygPrinting)
    {
        ...
        for( i1 = GetFixedColumnCount() + GetHeaderColumnCount() ; i1 < GetColumnCount(); i1++)
        {
	...
	//@**#---2002-12-18 16:01:42 (张军(ZhangJun))---#**@
...
void CGridCtrl::OnFilePrintPreview()
{
	CFrameWnd* pOldFrame = (CFrameWnd*)AfxGetApp()->GetMainWnd();
	if( !pOldFrame ) return ;
	pOldFrame->ShowWindow(SW_HIDE);
	if (!m_pTemplate)
	{
		m_pTemplate = new CSingleDocTemplate(
			129,					// so load fram is happy. This matches the IDR_MAINFRAME value from your application!
			NULL,
...
void CGridCtrl::OnPrint(CDC *pDC, CPrintInfo *pInfo)
{
    ...
    pDC->SetWindowOrg( - m_nLeftMargin / 25.4 * ScreenPixelsPerInch.cx, 0); // zj 2002.12.18 modified

    // Header
    pInfo->m_rectDraw.top    = m_nTopMargin / 25.4 * ScreenPixelsPerInch.cy;
    pInfo->m_rectDraw.left   = 0;
    pInfo->m_rectDraw.right  = m_LogicalPageSize.cx - (m_nLeftMargin + m_nRightMargin) / 25.4 * ScreenPixelsPerInch.cx;
    pInfo->m_rectDraw.bottom = ( m_nHeaderHeight  + m_nTopMargin ) / 25.4 * ScreenPixelsPerInch.cy;

    if(m_bPrintHeader)
		PrintHeader(pDC, pInfo);

    pDC->OffsetWindowOrg(0, - ( m_nHeaderHeight+m_nTopMargin) / 25.4 * ScreenPixelsPerInch.cy);

    // Gap between header and column headings
    pDC->OffsetWindowOrg(0, -m_nGap / 25.4 * ScreenPixelsPerInch.cy);

    if(!GetReport()) pDC->OffsetWindowOrg(0, -GetHeaderRowHeight()); // zj 2002.12.18 modified
	else pDC->OffsetWindowOrg(0, -GetFixedRowHeight()); // zj 2002.12.18 modified

    // We need to find out which row to start printing for this page.
    int nTotalRowHeight = 0;
    UINT nNumPages = 1;
    m_nCurrPrintRow = GetFixedRowCount() + GetHeaderRowCount(); // zj2002.12.18 Add
    ...
    m_nPrintColumn = GetFixedColumnCount() + GetHeaderColumnCount(); // zj 2002.12.18 modified
    int iTotalRowWidth = 0;
    int i1, i2;
	...
	PrintRowHeadings( pDC, pInfo);   // 打印行表标题
    ...
    if(!GetReport())
		pInfo->m_rectDraw.bottom = GetHeaderRowHeight(); // zj 2002.12.18 modified
	else
		pInfo->m_rectDraw.bottom = GetFixedRowHeight(); // zj 2002.12.18 modified

    if( m_nPrintColumn == GetFixedColumnCount() + GetHeaderColumnCount() )
    {
        // have the column headings fcn draw the upper left fixed cells
        //  for the very first columns, only
        if(!GetReport())
			pDC->OffsetWindowOrg( 0, +GetHeaderRowHeight());
		else
			pDC->OffsetWindowOrg( 0, +GetFixedRowHeight());

        m_nPageWidth += iColumnOffset;
        if(!GetReport()) m_nPrintColumn = GetFixedColumnCount();
		else m_nPrintColumn = GetHeaderColumnCount();
        PrintColumnHeadings(pDC, pInfo);
        m_nPageWidth -= iColumnOffset;

        m_nPrintColumn = GetFixedColumnCount()+GetHeaderColumnCount();

        if(!GetReport()) 
			pDC->OffsetWindowOrg( -iColumnOffset, -GetHeaderRowHeight());
		else
    ...
    // Footer
	pInfo->m_rectDraw.top    = 1;
    pInfo->m_rectDraw.bottom = (m_nFooterHeight ) / 25.4 * ScreenPixelsPerInch.cy;
    pDC->SetWindowOrg( -m_nLeftMargin / 25.4 * ScreenPixelsPerInch.cx,
        -m_LogicalPageSize.cy + ( m_nFooterHeight + m_nBottomMargin) / 25.4 * ScreenPixelsPerInch.cy);
...
// zj 200212.18 added
void CGridCtrl::PrintHeaderRowCells(int nStartColumn, int nStopColumn, int& row, CRect& rect,
                                   CDC *pDC, BOOL& bFirst)
{
   // print all cells from nStartColumn to nStopColumn on row
   for (int col =nStartColumn; col < nStopColumn; col++)
...
} // end of CGridCtrl::PrintHeaderRowCells
...
void CGridCtrl::PrintColumnHeadings(CDC *pDC, CPrintInfo* /*pInfo*/)
{
	...
	{		
		...
		{
			rect.top = rect.bottom+1;
			rect.bottom = rect.top + GetRowHeight(row + nFixedRows) - 1;
			...
			// added by M.Fletcher 12/17/00
			int tmp = row+nFixedRows;
			if(m_nPrintColumn>= GetFixedColumnCount() + GetHeaderRowCount())
			{
				bOriginal=bFirst;
				// lets print the missing fixed cells on left first out to last fixed column
				PrintHeaderRowCells(nFixedRows,nFixedRows+GetHeaderColumnCount(), tmp, rect, pDC, bFirst);
				bFirst=bOriginal;
			}
			
			// now back to normal business print cells in heading after all fixed columns
			PrintHeaderRowCells(m_nPrintColumn, GetColumnCount(), tmp, rect, pDC, bFirst);
	...
	{	
		...
		for (int row = 0; row < GetFixedRowCount(); row++)
		{
			...
			if(m_nPrintColumn>= GetFixedColumnCount() + GetHeaderRowCount())
			{
				bOriginal=bFirst;
				// lets print the missing fixed cells on left first out to last fixed column
				PrintHeaderRowCells(0,GetFixedColumnCount(), tmp, rect, pDC, bFirst);
				bFirst=bOriginal;
			}
...
void CGridCtrl::PrintRowHeadings(CDC *pDC, CPrintInfo* /*pInfo*/)
{
    CFont *pOldFont = pDC->SelectObject(&m_PrinterFont);
	...
	else
	{
		...
		int nFixedCols = GetFixedColumnCount();
		for( int iCol = 0; iCol < GetHeaderColumnCount(); iCol++)
		{
			rect.left = rect.right+1;
			rect.right =    rect.left
				+ GetColumnWidth( iCol + nFixedCols )
				- 1;
			...
			for( int iRow = m_nCurrPrintRow; iRow < GetRowCount(); iRow++)
			{
				...
				CGridCellBase* pCell = GetCell(iRow, iCol + nFixedCols );
				if (pCell)
					pCell->PrintCell(pDC, iRow, iCol + nFixedCols , rect);
				
				if (m_nGridLines == GVL_BOTH || m_nGridLines == GVL_HORZ)
				{
					int Overlap = ( ( iCol + nFixedCols) == 0)? 0:1;
					pDC->MoveTo(rect.left-Overlap, rect.bottom);
					pDC->LineTo(rect.right, rect.bottom);
					if( bFirst) {
						pDC->MoveTo(rect.left-Overlap, rect.top-1);
...
void CGridCtrl::PrintHeader(CDC *pDC, CPrintInfo *pInfo)
{
	...
	AnalyPrintFormat(m_strHeaderFormat, strLeft, strCenter, strRight, pInfo);
...
void CGridCtrl::PrintFooter(CDC *pDC, CPrintInfo *pInfo)
{
	...
	AnalyPrintFormat(m_strFooterFormat, strLeft, strCenter, strRight, pInfo);

    CRect rc(pInfo->m_rectDraw);

	if ( m_bPrintFooterLine )
	{
...
BOOL CGridCtrl::Save(LPCTSTR filename, TCHAR chSeparator/*=_T(',')*/)
{
    ...
    {
        int nNumColumns = GetColumnCount();
		for (int i = (m_bPrintFixedRow?0:1); i < GetRowCount(); i++)
        {
            for (int j = (m_bPrintFixedCol?0:1); j < nNumColumns; j++)
            {
...
// zj 2004.6.21 ADD
BOOL CGridCtrl::SaveToExcelFile(LPCTSTR filename)
{
	CFile file;
	...
	int nNumColumns = GetColumnCount();
	int nNumRows = GetRowCount();
	for (int i = (m_bPrintFixedRow?0:1); i < nNumRows; i++)
	{
		for (int j = (m_bPrintFixedCol?0:1); j < nNumColumns; j++)
		{
			CGridCellBase * pCellBase = GetCell(i, j);
			DWORD nFormat = pCellBase->GetFormat();
			DWORD format = 0;
			...
			if( nFormat & DT_CENTER )
				format |= ALIGN_CENTER;
			miniexcel(i-(m_bPrintFixedRow?0:1),j-(m_bPrintFixedCol?0:1)).setAlignament(format);

			int bdline = pCellBase->GetCellBorderLine();
			int border = 0;
			if(bdline & CBL_LEFT)
				border |= BORDER_LEFT;
			if(bdline & CBL_RIGHT)
			...
			if(bdline & CBL_TOP)
				border |= BORDER_TOP;
			if(bdline & CBL_BOTTOM)
				border |= BORDER_BOTTOM;

			miniexcel(i-(m_bPrintFixedRow?0:1),j-(m_bPrintFixedCol?0:1)).setBorder(border);

			switch(pCellBase->m_nType) {
			case 5:
				{
					CGridCellNumeric *p = (CGridCellNumeric*)pCellBase;
					miniexcel(i-(m_bPrintFixedRow?0:1),j-(m_bPrintFixedCol?0:1)) = p->GetTextAsLong();
				}				
				
				break;
			case 6:
				{
					CGridCellDouble *p = (CGridCellDouble*)pCellBase;
					miniexcel(i-(m_bPrintFixedRow?0:1),j-(m_bPrintFixedCol?0:1)) = p->GetTextAsDouble();
				}
				
				break;
			case 7:
				{
					CGridCellCurrency *p = (CGridCellCurrency*)pCellBase;
					miniexcel(i-(m_bPrintFixedRow?0:1),j-(m_bPrintFixedCol?0:1)) = p->GetTextAsDouble();
				}
				break;
			default:
				miniexcel(i-(m_bPrintFixedRow?0:1),j-(m_bPrintFixedCol?0:1)) = GetItemText(i,j);
				break;
			}			
	...
	else
	{
		for(int i=range.GetMinRow(); i<=range.GetMaxRow(); i++)
		{
			for(int j = range.GetMinCol(); j<=range.GetMaxCol(); j++)
			{
				...
				if( pCell->IsMerged() )
				{
					CCellRange MergeRange = pCell->GetMergeRange();
					if( MergeRange.GetMinCol() < range.GetMinCol() 
					 || MergeRange.GetMaxCol() > range.GetMaxCol()
					 || MergeRange.GetMinRow() < range.GetMinRow()
					 || MergeRange.GetMaxRow() > range.GetMaxRow() )
					 return;
				}
			}
		}
...
CDocument* CGridCtrl::GetDocument()
{
	return m_pDocument;
}

void CGridCtrl::SetDocument(CDocument* pDoc)
{
	m_pDocument = pDoc;
}

void CGridCtrl::AnalyPrintFormat(CString &Format, 
								 CString &left, CString &center, CString &right,
								 CPrintInfo *pInfo)
{
	int n = Format.GetLength();
	...
	for( int i = 0; i < n; i++)
	{
		...
		if( c=='&' && n > (i+1) && Format[i+1] == 'p')
		{
			temp.Format(_T("%d"), pInfo->m_nCurPage );
			i++;			
		}

		else if( c=='&' && n > (i+1) && Format[i+1] == 't' )
		{
			temp.Format(_T("%d"), pInfo->GetMaxPage() );
			i++;			
		}
		else if( c=='&' && n > (i+1) && Format[i+1] == 'f' )
		{			
			CWnd *pParentWnd = GetParent();
			while (pParentWnd)
			{
				pParentWnd->GetWindowText(temp);
				if (temp.GetLength())  // can happen if it is a CView, CChildFrm has the title
					break;
				pParentWnd = pParentWnd->GetParent();
			}
			i++;
...
{
	int nCols, nRows, nFixedCols, nFixedRows, nHeaderCols, nHeaderRows;
	int width, height;	
	int nColAddEnable, nRowAddEnable;
	int i,j;
	int nGridLines;
	
	BOOL bListMode;
	BOOL bSelectable;
	BOOL bReSizeCol, bReSizeRow;
	BOOL bDrag, bTip;

	if(ar.IsLoading())
	{
		ar >> nCols >> nRows >> nFixedCols >> nFixedRows >> nHeaderCols >> nHeaderRows;
		ar >> bListMode >> bSelectable;
		ar >> bReSizeCol >> bReSizeRow;
		ar >> bDrag;
		ar >> bTip;
		ar >> nGridLines;
		...
		SetFixedColumnCount(nFixedCols);
		SetFixedRowCount(nFixedRows);
		SetHeaderColumnCount(nHeaderCols);
		SetHeaderRowCount(nHeaderRows);
		
		SetListMode(bListMode);
		EnableSelection(bSelectable);
		
		SetColumnResize(bReSizeCol);
		SetRowResize(bReSizeRow);
		EnableTitleTips(bTip);
		EnableDragAndDrop(bDrag);
		
		SetGridLines(nGridLines);
		...
		for( i = 0 ; i < nCols ; i++)
		{
			...
			ar >> nColAddEnable;
			SetColAddEnable(i,nColAddEnable); //for 101
		}
		...
		{			
			...
			ar >> nRowAddEnable;
			SetRowAddEnable(i,nRowAddEnable); //for101
		}
		...
		for( i = 0; i< nRows; i++)
		{
			for( j =0; j<nCols;j++)
			{
				...
				{
				case 1:
					SetCellType(i, j, RUNTIME_CLASS(CGridCellDateTime));
					break;
				case 2:
					SetCellType(i, j, RUNTIME_CLASS(CGridCellCheck));
					break;
				case 3:
					SetCellType(i, j, RUNTIME_CLASS(CGridCellCheckCombo));
					break;
				case 4:
					SetCellType(i, j, RUNTIME_CLASS(CGridCellCombo));
					break;
				case 5:
					SetCellType(i, j, RUNTIME_CLASS(CGridCellNumeric));
					break;
				case 6:
					SetCellType(i, j, RUNTIME_CLASS(CGridCellDouble));
					break;
				case 7:
					SetCellType(i, j, RUNTIME_CLASS(CGridCellCurrency));
					break;
				case 8:
					SetCellType(i, j, RUNTIME_CLASS(CGridCellMask));
					break;
					/*case 9:
					m_Grid.SetCellType(i, j, RUNTIME_CLASS(CGridCellDouble));
					break;*/
				default:
					break;
				}
		...
		ar >> cellID.col >> cellID.row;
		
		SetFocusCell(cellID);
		
		CGridCellBase * p = GetDefaultCell(FALSE, FALSE);
	...
	else
	{
		...
		nRows = GetRowCount();
		nFixedCols = GetFixedColumnCount();
		nFixedRows = GetFixedRowCount();
		nHeaderCols = GetHeaderColumnCount();
		nHeaderRows = GetHeaderRowCount();
		
		bListMode = GetListMode();
		bSelectable = IsSelectable();
		
		bReSizeCol = GetColumnResize();
		bReSizeRow = GetRowResize();
		bDrag = GetDragAndDrop();
		bTip = GetTitleTips();
		
		nGridLines = GetGridLines();
		...
		for( i = 0 ; i < nCols ; i++)
		{
			...
			nColAddEnable = GetColAddEnable(i); // for 101
			ar << nColAddEnable;
		}
		...
		for( i = 0 ; i < nRows ; i++)
		{
			...
			nRowAddEnable = GetRowAddEnable(i); // for 101
			ar << nRowAddEnable;
		}
...
void CGridCtrl::SetPrintFixedRow(const BOOL &bValue)
{
	m_bPrintFixedRow = bValue;
}

void CGridCtrl::SetPrintFixedCol(const BOOL &bValue)
{
	m_bPrintFixedCol = bValue;
}

BOOL CGridCtrl::GetPrintFixedRow()
{
	return m_bPrintFixedRow;
}

BOOL CGridCtrl::GetPrintFixedCol()
{
	return m_bPrintFixedCol;
}

void CGridCtrl::SetAutoAddRow(const BOOL &bValue)
{
	m_bAutoAddRow = bValue;
}

BOOL CGridCtrl::GetAutoAddRow()
{
	return m_bAutoAddRow;
}
void CGridCtrl::SetAutoAddCol(const BOOL &bValue)
{
	m_bAutoAddCol = bValue;
}

BOOL CGridCtrl::GetAutoAddCol()
{
	return m_bAutoAddCol;
}
