...
class CGraphWnd : public CWnd, public virtual CGraphBaseClass
{
	// Construction
public:
	CGraphWnd(long maximum_graphs = 100);
...
public:
	...
	BOOL Create(LPCTSTR lpszWindowName, const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwStyle = WS_CHILD | WS_VISIBLE, BOOL bAutoUpdateTimer = FALSE);

	//operations with graphs
	virtual int AddGraph(COLORREF newColor = 0, const TCHAR* title = _T(""), BOOL bRedraw = TRUE, BOOL b_sort_x = TRUE, BOOL b_keep_same_x = FALSE);
	...
	{
		if( graph_frame )
			graph_frame->SetCurrentGraph(index);
	}
	void SetCurrentGraph(LPCTSTR lpszGraph)
	{
		if( graph_frame )
			graph_frame->SetCurrentGraph(lpszGraph);
	}
	...
	CGraphPanel* GetGraphPanel()
	{
		if( graph_frame != NULL )
			return graph_frame->graph_container.graph_panel;
		else
			return NULL;
	}

	CGraphContainer* GetContainer()
	{
		if( graph_frame != NULL )
			return &graph_frame->graph_container;
		else
			return NULL;
	}

	CPointViewBar* GetPointViewBar()
	{
		if( graph_frame != NULL )
			return &graph_frame->pview_bar;
		else
			return NULL;
	}
