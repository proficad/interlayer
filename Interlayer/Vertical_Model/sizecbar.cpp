...
void CSizingControlBar::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS FAR* lpncsp)
{
	...
	if (!IsFloating() && m_pDockBar != NULL)
	{
		...
		for (int i = 0; i < arrSCBars.GetSize(); i++)
			...
			{
				if (i > 0)
					m_dwSCBStyle |= IsHorzDocked() ? SCBS_EDGELEFT : SCBS_EDGETOP;
				if (i < arrSCBars.GetSize() - 1)
					m_dwSCBStyle |= IsHorzDocked() ? SCBS_EDGERIGHT : SCBS_EDGEBOTTOM;
			}
	}

	// make room for edges only if they will be painted
	if (m_dwSCBStyle & SCBS_SHOWEDGES)
		rcClient.DeflateRect((m_dwSCBStyle & SCBS_EDGELEFT) ? m_cxEdge : 0, (m_dwSCBStyle & SCBS_EDGETOP) ? m_cxEdge : 0, (m_dwSCBStyle & SCBS_EDGERIGHT) ? m_cxEdge : 0, (m_dwSCBStyle & SCBS_EDGEBOTTOM) ? m_cxEdge : 0);

	// "hide" button positioning
	CPoint ptOrgBtn;
	if (IsHorzDocked())
		ptOrgBtn = CPoint(rcClient.left - m_cyGripper - 1, rcClient.top - 1);
	else
		ptOrgBtn = CPoint(rcClient.right - 11, rcClient.top - m_cyGripper - 1);
	...
	if (m_dwSCBStyle & SCBS_SHOWEDGES)
	{
		...
		for (int i = 0; i < 4; i++)
			if (GetEdgeRect(rcBar, GetEdgeHTCode(i), rcEdge))
				dc.Draw3dRect(rcEdge, ::GetSysColor(COLOR_BTNHIGHLIGHT), ::GetSysColor(COLOR_BTNSHADOW));
	}
...
void CSizingControlBar::StartTracking(UINT nHitTest)
{
	...
	else
	{
		...
		m_szMaxT = m_szOld + (bHorz ? pBar->m_szHorz : pBar->m_szVert) - pBar->m_szMin;
	}
...
void CSizingControlBar::OnTrackUpdateSize(CPoint& point)
{
	...
	{
	case HTLEFT:
		sizeNew -= CSize(szDelta.cx, 0); break;
	case HTTOP:
		sizeNew -= CSize(0, szDelta.cy); break;
	case HTRIGHT:
		sizeNew += CSize(szDelta.cx, 0); break;
	case HTBOTTOM:
		sizeNew += CSize(0, szDelta.cy); break;
	...
	for (int i = 0; i < arrSCBars.GetSize(); i++)
		...
		else
		{
			...
			(bHorz ? pBar->m_szHorz.cx : pBar->m_szVert.cy) -= bHorz ? szDelta.cx : szDelta.cy;
		}
...
void CSizingControlBar::OnTrackInvertTracker()
{
	...
	if (m_nDockBarID == AFX_IDW_DOCKBAR_LEFT && m_htEdge == HTTOP || m_nDockBarID == AFX_IDW_DOCKBAR_RIGHT && m_htEdge != HTBOTTOM || m_nDockBarID == AFX_IDW_DOCKBAR_TOP && m_htEdge == HTLEFT || m_nDockBarID == AFX_IDW_DOCKBAR_BOTTOM && m_htEdge != HTRIGHT)
		sizeDelta = -sizeDelta;
...
BOOL CSizingControlBar::NegociateSpace(int nLengthAvail, BOOL bHorz)
{
	...
	{
		pBar = arrSCBars[i];
		nActualLength += bHorz ? pBar->m_szHorz.cx - 2 : pBar->m_szVert.cy - 2;
		nMinLength += bHorz ? pBar->m_szMin.cx - 2 : pBar->m_szMin.cy - 2;
		nWidth = max(nWidth, bHorz ? pBar->m_szHorz.cy : pBar->m_szVert.cx);
	}
...
void CSizingControlBar::AlignControlBars()
{
	...
	for (int i = nFirst; i <= nLast; i++)
	{
		...
		if ((nPos = FindSizingBar(pBar)) >= 0)
			rc = CRect(rc.TopLeft(), bHorz ? m_arrBars[nPos]->m_szHorz : m_arrBars[nPos]->m_szVert);
