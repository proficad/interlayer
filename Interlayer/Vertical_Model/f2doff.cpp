...
void FOffScreen::DrawDib(HDC dc, CRect r)
{
	if (Dib != NULL)
	{
		SetDIBitsToDevice(dc, r.left, r.top, r.Width(), r.Height(), r.left, fbmi->bmiHeader.biHeight - r.bottom, 0, fbmi->bmiHeader.biHeight, Dib, fbmi, DIB_RGB_COLORS);
	};
}
	...
	if (cx != 0)
	{
		...
		if (abs(cx) < Width)
		{
			for (int i = 0; i < Height; i++)
			{
				...
				if (cx < 0)
					memcpy(lp2, lp1, Width - abs(cx));
				else
					memcpy(lp1, lp2, Width - abs(cx));
			};
		};
	};
	if (cy != 0)
